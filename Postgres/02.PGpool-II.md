...
failover_command = '/etc/pgpool-II/failover.sh %d %P %H %M %R'
#failback_command = '/etc/pgpool-II/failback.sh %d %P %H %M %R'
...
NEW_PRIMARY_HOST="$3"
NEW_PRIMARY_PORT="$4"
NEW_PRIMARY_DATADIR="$5"
NEW_PRIMARY_NODE_ID="$6"
...
logger -t $LOGTAG "Failed node: $FAILED_HOST (node_id=$FAILED_NODE_ID)"
logger -t $LOGTAG "Promoting new primary: $NEW_PRIMARY_HOST"

# Promote standby
ssh -o StrictHostKeyChecking=no postgres@"$NEW_PRIMARY_HOST" \
  "$PG_CTL promote -D $NEW_PRIMARY_DATADIR"

if [ $? -ne 0 ]; then
  logger -t $LOGTAG "ERROR: Promotion failed on $NEW_PRIMARY_HOST"
  exit 1
fi

logger -t $LOGTAG "Promotion successful on $NEW_PRIMARY_HOST"
